...
static unsigned int am_compile_shader(unsigned int type, const char& source){
    unsigned int id = glCreateShader(GL_VERTEX_SHADER);
    const char* src = source.c_str();
}

static int am_create_shader(const char& vertex_shader, const chat& fragment_shader){
    unsigned int program = glCreateProgram();
    unsigned int vs = am_compile_shader(GL_VERTEX_SHADER, vertex_shader);
